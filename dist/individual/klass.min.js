...
var _super=this.prototype,klassIdent=(klassName||""),klassPath=klassIdent.split('.'),klassName=klassPath.pop(),klassMeths=prop['klass'],supKlasMeths=this['_staticMethods'],ctx=window;delete prop['klass'];for(var i=0;i<klassPath.length;i++){var pathSeg=klassPath[i];if(!ctx[pathSeg])ctx[pathSeg]={};ctx=ctx[pathSeg];};klassPath=klassPath.join('.');initializing=true;var prototype=new this();initializing=false;Klass._name=klassName;Klass._path=klassPath;Klass._fullname=klassIdent;Klass._staticMethods=klassMeths;prototype._klass=Klass;mergeProperties(prop,prototype,_super,this);function Klass(klassName,props){if(this instanceof arguments.callee){if(!initializing&&this.init)
this.init.apply(this,arguments);}else{arguments.callee.subclassAs(klassName,props)}}
Klass.prototype=prototype;Klass.constructor=Klass;if(supKlasMeths)mergeProperties(supKlasMeths,Klass,{},Klass);mergeProperties(klassMeths,Klass,(supKlasMeths||{}),Klass);Klass.subclassAs=arguments.callee;ctx[klassName]=Klass;return Klass;};})();